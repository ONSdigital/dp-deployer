...
func main() {
	...
	if err != nil {
		log.Event(ctx, "bad init handlers", log.Data{"err": err})
		os.Exit(1)
	}
	...
	if err != nil {
		log.Event(ctx, "bad new engine", log.Data{"err": err})
		os.Exit(1)
	}
	...
	if err != nil {
		log.Event(ctx, "bad health interval", log.Data{"err": err})
		os.Exit(1)
	}

	healthTimeout, err := time.ParseDuration(*healthcheckCriticalTimeout)
	if err != nil {
		log.Event(ctx, "bad health timeout", log.Data{"err": err})
		os.Exit(1)
	}
	...
	if err != nil {
		log.Event(ctx, "bad health version", log.Data{"err": err})
		os.Exit(1)
	}
	...
	go func() {
		if err := httpServer.ListenAndServe(); err != nil {
			log.Event(ctx, "bad http listen", log.Data{"err": err, "config": hcc})
			cancel()
		}
	}()
